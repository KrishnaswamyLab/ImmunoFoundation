###########################################################################################
# Geometric Vector Perceptron (GVP) layers for equivariant graph neural networks.
...
def tuple_cat(*args, dim=-1):
    ...
    dim=-1 for scalars becomes dim=-2 for vectors.
    """
...
def _merge(s, v):
    """Merges a tuple (s, V) into a single tensor (flattened vectors appended to scalars)."""
...
class GVPConv(MessagePassing):
    ...
    Takes in a graph with node and edge embeddings, returns new node embeddings.
    """
...
class GVPConvLayer(nn.Module):
    """
    Full graph convolution / message passing layer with GVP.
    Residually updates node embeddings with aggregated incoming messages,
    applies a pointwise feedforward network, and returns updated node embeddings.
    """
