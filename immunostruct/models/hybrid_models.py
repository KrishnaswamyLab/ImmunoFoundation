...
from .scatter_deshaw import Scatter_deshaw
import numpy as np

__all__ = ["HybridModel", "HybridModelv2", "HybridModel_SSL", "HybridModelv2_SSL", "HybridModelv3"]
...
class HybridModelv3(nn.Module):
    def __init__(self,
        ...
        #Geometric scattering components
        self.geometric_scattering = Scatter_deshaw(in_channels=26,
                                                 max_graph_size=1000,
                                                 trainable_laziness=False,
                                                 trainable_f=True)
        self.scatter_projection = nn.Linear(self.geometric_scattering.out_shape(),
    ...
    def forward(self, graph_data, sequence_data, peptide_property, return_embedding=False, return_attention=False):
        ...
        #temp_data.x = node_feat
        #combined_features = torch.cat([node_feat, coord_feat], dim=1)
        #temp_data.x = combined_features
        #Convert DGL edges to PyG format for deshaw_scatter
        src, dst = graph_data.edges()
        temp_data.edge_index = torch.stack([src, dst], dim=0)
        temp_data.x = torch.cat([node_feat, graph_data.ndata['internal_coords']], dim=1)
        temp_data.edge_attr = edge_feat.flatten()
        temp_data.batch = batch_tensor.flatten()
        temp_data.num_graphs = batch_tensor.shape[0]
        scatter_output = self.geometric_scattering(temp_data)
        node_feat = self.scatter_projection(scatter_output)
